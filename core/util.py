...
def get_settings(conf_name="config.yml", set_env=True):
    ...
    """
    try:
        with open(os.path.join(dsettings.BASE_DIR, "core", conf_name), "r") as f:
...
def read_settings_app():
    ...
    if "app" not in settings:
        log.error(
            "Application settings are not present in the config file. Add app section, see the doc"
        )
...
def get_create_plan_cmd(settings):
    cmd = (
        "az appservice plan create --sku {sku} --is-linux -l "
        + "eastus2 -n {plan_name} -g {resource_group}"
    ).format(**settings["app"])
...
def get_webapp_cmd(settings):
    cmd = (
        "az webapp create -n {app_name} -g {resource_group} "
        + "--plan {plan_name} -i {container_name}"
    ).format(**settings["app"])
...
def get_restart_cmd(settings):
    cmd = ("az webapp restart -g {resource_group} -n {app_name}").format(
        **settings["app"]
    )
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "run_docker":
        ...
        print(
            "Usage: python util.py run_docker|create_app|delete_app|setenv [-d|--dry]"
        )
