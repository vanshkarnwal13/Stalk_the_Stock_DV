...
mongoclient = "mongodb+srv://happyadmin:"+ urllib.parse.quote_plus("Happy@123")+"@cluster0.eyb8f.mongodb.net/happyinvesting?retryWrites=true&w=majority"
...
for symbol in all_stock_codes:
    try: 
        dump = pd.DataFrame(list(db[cash_collection].find({"TIMESTAMP":{"$gte":start_date,"$lte":end_date},"SYMBOL":symbol})))
        dump10 = pd.DataFrame(list(db[cash_collection].find({"TIMESTAMP":{"$gte":start_date10,"$lte":end_date},"SYMBOL":symbol})))
        dump1M = pd.DataFrame(list(db[cash_collection].find({"TIMESTAMP":{"$gte":start_date1M,"$lte":end_date},"SYMBOL":symbol})))
